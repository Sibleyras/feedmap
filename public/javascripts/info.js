...
class Info {
  ...
  constructor(properties) {
    ...
    if (properties && properties["createdAt"])
      this.info["timediff"] =
        (currtime.getTime() - new Date(properties["createdAt"]).getTime()) /
        1000;
  ...
  updateValues(fields) {
    for (var k of this.keys) {
      if (fields[k]) this.info[k] = fields[k];
    }
  }
  ...
  getDisplay() {
    if (!this.display) this.initDisplay();
    return this.display;
  }
  ...
  buildFormulaire() {
    if (!this.info["marker"]) this.info["marker"] = Object.keys(iconUrls)[0];
    if (!this.form) this.initForm();
    if (!this.overlaypicker) this.buildPickOverlay();

    this.setMarker(this.info["marker"]);

    //fill with data if exists
    if (this.info["description"])
      this.formInputs["description"].value = this.info["description"];
    if (this.info["source"])
      this.formInputs["source"].value = this.info["source"];
  ...
  getNewform() {
    ...
    if (!this.newform) this.initNewform();
    return this.newform;
  }
  ...
  initEditform() {
    ...
    submitbutton.addEventListener("click", () => this.editToDB());
  ...
  buildPickOverlay() {
    ...
    this.overlaypicker.addEventListener(
      "click",
      () => (this.overlaypicker.style.display = "none")
    );

    var selectflex = document.createElement("div");
    selectflex.classList.add(
      ...["row", "d-flex", "justify-content-center", "vertical-center"]
    );
    ...
    this.formInputs["marker"].addEventListener(
      "click",
      () => (this.overlaypicker.style.display = "block")
    );
  ...
  infoTarget() {
    if (targetInfo) targetInfo.unTarget();
  ...
  getTimediff() {
    ...
    var dhms = [
      Math.floor(this.info["timediff"] / (3600 * 24)),
    ...
    if (!dhms[--i]) return "<p>À l'instant</p>";
  ...
  removeInfo() {
    this.removeMarker();
    if (this.overlaypicker) this.overlaypicker.remove();
  }
  ...
  editToDB() {
    this.updateFromForm();
    $.ajax({
      url: "/api/info/" + this.info["id"],
      type: "PUT",
      success: function (data) {
        refreshInfoList();
        if (data === "success") editoverlay.firstChild.innerHTML = "Entrée modifiée.";
        else editoverlay.firstChild.innerHTML = data;
      },
      failure: function (errMsg) {
        alert(errMsg);
        console.log(errMsg);
  ...
  supprFromDB() {
    $.ajax({
      ...
      success: function (data) {
        refreshInfoList();
        if (data === "success") {
          editoverlay.firstChild.innerHTML = "Entrée supprimée.";
        } else if (data === "notfound") {
          editoverlay.firstChild.innerHTML = "Entrée inexistante.";
        } else {
